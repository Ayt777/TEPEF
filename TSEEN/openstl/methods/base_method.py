...
class CustomMSELoss(nn.Module):
    ...
    def forward(self, pred_y, batch_y):
        ...
        # 将权重张量扩展到与 pred_y 和 batch_y 相同的形状
...
class Base_method(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # print((pred_y).mean(),(batch_y).mean())
        current_epoch = self.current_epoch
        # alpha = math.exp(-1*current_epoch/50)
        # 4->4
        alpha = math.exp(-0.2*current_epoch/10)
        loss = self.criterion(pred_y, batch_y)
        loss = loss + alpha * 10 * self.diff_div_reg(pred_y, batch_y)
        # 4->8
        # alpha = math.exp(-0.2*current_epoch/12)
        # loss = self.criterion(pred_y, batch_y)
        # loss = loss + alpha * 10 * self.diff_div_reg(pred_y, batch_y)
        # 4->16
        # alpha = math.exp(-0.2*current_epoch/15)
        # loss = self.criterion(pred_y, batch_y)
        # loss = loss + alpha * 1 * self.diff_div_reg(pred_y, batch_y)
